...
client = genai.Client(api_key="AIzaSyCGRPhI1CaMSnjy6QPlLE5Y9EaqX52IyQo")
...
def scan_face():
    try:
        ...
        all_pets = c.fetchall()
        conn.close()
        best_match, highest_score, threshold = None, 0, 0.55 
        for pet in all_pets:
            file_path = os.path.join(UPLOAD_FOLDER, pet[4])
            score = compare_faces(captured_image, file_path)
            if score > highest_score: highest_score, best_match = score, pet
        if best_match and highest_score >= threshold:
            img_path = os.path.join(UPLOAD_FOLDER, best_match[4])
            with open(img_path, "rb") as f:
                img_str = "data:image/jpeg;base64," + base64.b64encode(f.read()).decode('utf-8')
            return jsonify({"match": True, "data": {"name": pet[1], "owner": pet[2], "photo": img_str}})
        return jsonify({"match": False})
    except Exception as e: return jsonify({"match": False, "message": str(e)})

@app.route('/health_scan', methods=['POST'])
def health_scan():
    try:
        data = request.json
        header, encoded = data['image'].split(",", 1)
        ...
        response = client.models.generate_content(
            ...
            contents=[
                "Analyze this dog's health. Respond ONLY in valid JSON format: {\"condition\": \"...\", \"symptoms\": \"...\", \"remedy\": \"...\", \"confidence\": 0}",
            ]
        )
        ...
        return jsonify({"status": "success", **json.loads(raw_text)})
